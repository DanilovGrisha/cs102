...
def greeting(message):
    bot.send_message(message.chat.id, 'Тестовый бот по расписанию занятий.')
...
def get_resp(web_page, day_number: str):
    times_lst, locations_lst, lessons_lst = parse_schedule_for_a_day(
        web_page, day_number)
    resp = ''
    for time, location, lesson in zip(times_lst, locations_lst, lessons_lst):
        resp += '<b>{}</b>, {}'.format(time, lesson)
    return resp
    ...
    for i in range(1, 7):
        ...
        if parse_schedule_for_a_day(get_page(group), day_number) is None:
            ...
            mess += days[day_number] + \
                get_resp(get_page(group), day_number)
